...
export const otpVerify = createAsyncThunk(
  ...
  async ({ otp }, { rejectWithValue }) => {
    try {
      ...
      if (
        res.message === "OTP verified, registration successful" ||
  ...
  async ({ email, password, toast }, { rejectWithValue }) => {
    try {
      ...
      if (error.response.data.message == "Account blocked") {
        ...
        toast.error("Your Account Blocked", {
...
export const editProfile = createAsyncThunk(
  ...
  async ({ values }, { rejectWithValue }) => {
    try {
      ...
      ); // Reject and pass the error back
    }
  }
);
...
export const rateProperty = createAsyncThunk(
  ...
  async ({ userId, proId, rate, review }, thunkAPI) => {
    try {
      // console.log(userId);
      const res = await UserResService.ratingProperty(
        userId,
        proId,
        rate,
        review
