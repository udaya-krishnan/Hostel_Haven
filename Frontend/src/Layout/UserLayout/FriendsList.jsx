...
const FriendsList = ({ onSelectFriend }) => {
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      ...
      console.log(res.payload.response, "response");
  ...
  return (
    <div className="w-1/4 bg-gray-100 p-4">
      ...
      {/* Conditional rendering when no connections are available */}
      {connections.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <p className="text-gray-500 text-sm">
            Start a conversation and connect with hosts or users!
          </p>
          {/* <div className="mt-4">
            <img
              src="https://via.placeholder.com/150/000000/FFFFFF?text=No+Friends"
              alt="No friends"
        ...
        <ul>
          {connections.map((connect, index) => (
            ...
            >
              {console.log(connect.host_id, "host ids")}
              <div className="w-12 h-12 bg-gray-400 rounded-full">
                <img
                  src={
                    connect.host_id.image || "https://via.placeholder.com/50"
                  }
              ...
              <div>
                ...
                {connect.messages.length > 0 ? (
                  <p className="text-sm text-gray-500">
                    Last message:{" "}
                    {getTime(
                      connect.messages[connect.messages.length - 1].timestamp
                    )}
                  </p>
                ) : (
